...
function BankDetail() {
    ...
    useEffect(() => {
        async function fetchBankDetails() {
            try {
                const response = await axios.get(`https://www.testapi.helpubuild.in/api/v1/get-single-provider/${UserId}`);
    ...
    const handleSubmit = async (e) => {
        ...
        setLoading(true);
        try {
            const response = await axios.put(`https://www.testapi.helpubuild.in/api/v1/update-bank-detail/${UserId}`, bankDetail);
            if (response.data.success) {
