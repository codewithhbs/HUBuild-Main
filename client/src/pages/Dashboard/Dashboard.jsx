...
function Dashboard() {
  ...
  const handleCropComplete = async (blob) => {
    ...
    try {
      const res = await axios.put(`https://www.testapi.helpubuild.in/api/v1/update_user_profile_image/${userId}`, formData)
      if (res.data.success) {
  ...
  const handleCheckCoupon = async () => {
    ...
    try {
      const res = await axios.post("https://www.testapi.helpubuild.in/api/v1/check_coupon", {
  ...
  const handleMakePayment = async () => {
    ...
    try {
      ...
      const res = await axios.post(`https://www.testapi.helpubuild.in/api/v1/create-payment/${UserId}`, requestBody)
  ...
  const handleDeleteAccount = async (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const res = await axios.delete(`https://www.testapi.helpubuild.in/api/v1/user-delete/${id}`)
          if (res.data.success) {
