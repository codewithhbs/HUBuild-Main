...
const UserDashboard = () => {
  ...
  const handleFetchProvider = async () => {
    try {
      ...
      const { data } = await axios.get(
        `https://www.testapi.helpubuild.in/api/v1/get-single-provider/${providerId}`
      );
  ...
  const handleToggle = async (statusType) => {
    ...
    try {
      const response = await axios.put(
        `https://www.testapi.helpubuild.in/api/v1/update-available-status/${providerId}`,
  ...
  const handleUpload = async () => {
    ...
    try {
      const response = await axios.post('https://www.testapi.helpubuild.in/api/v1/addPortfolio?type=Portfolio', formData, {
        headers: {
  ...
  const handleIsDeactived = async (id, isDeactived) => {
    try {
      const res = await axios.patch(`https://www.testapi.helpubuild.in/api/v1/update-provider-deactive-status/${id}`)
      if (res.data.success) {
  ...
  const handleCropComplete = async (blob) => {
    ...
    try {
      const res = await axios.put(`https://www.testapi.helpubuild.in/api/v1/update_provider_profile_image/${providerId}`, formData)
      if (res.data.success) {
  ...
  const handleDeleteAccount = async (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const res = await axios.delete(`https://www.testapi.helpubuild.in/api/v1/delete-provider/${id}`)
          if (res.data.success) {
            localStorage.clear()
  ...
  const handleFetchCommission = async () => {
    try {
      const { data } = await axios.get('https://www.testapi.helpubuild.in/api/v1/get-all-commision')
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post("https://www.testapi.helpubuild.in/api/v1/create-withdraw-request", {
