...
const Footer = () => {
  ...
  const getAllTermForFooter = async () => {
    try {
      const { data } = await axios.get('https://www.testapi.helpubuild.in/api/v1/all_term');
  ...
  const handleNewsletterSubmit = async (e) => {
    e.preventDefault();
    try {
      const res = await axios.post('https://www.testapi.helpubuild.in/api/v1/create_newletter', { email });
