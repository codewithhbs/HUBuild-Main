...
const EditExpertise = () => {
  ...
  const fetchExpertiseDetails = async () => {
    try {
      const { data } = await axios.get(`http://localhost:5000/api/v1/single_expertise/${id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await axios.put(`http://localhost:5000/api/v1/update_expertise/${id}`, { expertise });
