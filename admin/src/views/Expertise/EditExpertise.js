...
const EditExpertise = () => {
  ...
  const fetchExpertiseDetails = async () => {
    try {
      const { data } = await axios.get(`https://testapi.helpubuild.in/api/v1/single_expertise/${id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await axios.put(`https://testapi.helpubuild.in/api/v1/update_expertise/${id}`, { expertise });
