...
const AllCustomChat = () => {
    ...
    const handleFetchChat = async (chatRoomId) => {
        try {
            const { data } = await axios.get(`https://www.testapi.helpubuild.in/api/v1/get-group-chat-by-id/${chatRoomId}`);
    ...
    const handleUpdateIsChatEnded = async (id, isChatEnded) => {
        ...
        try {
            const { data } = await axios.put(
                `https://www.testapi.helpubuild.in/api/v1/update_manual_chat_ended/${id}`,
    ...
    const handleDeleteBanner = async (id) => {
        setLoading(true);
        try {
            await axios.delete(`https://www.testapi.helpubuild.in/api/v1/delete-chat-room/${id}`);
            handleFetchBanner();
