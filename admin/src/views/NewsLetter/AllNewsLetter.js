...
const AllNewsLetter = () => {
    ...
    const fetchNewsletters = async () => {
        setLoading(true);
        try {
            const { data } = await axios.get('https://api.helpubuild.in/api/v1/all_newsletter');
    ...
    const handleSendNewsLetter = async () => {
        setLoading(true);
        try {
            const response = await axios.post('https://api.helpubuild.in/api/v1/send_message_newsletter', { message });
    ...
    const handleDeleteNewsletter = async (id) => {
        setLoading(true);
        try {
            await axios.delete(`https://api.helpubuild.in/api/v1/delete_newsletter/${id}`);
            fetchNewsletters();
