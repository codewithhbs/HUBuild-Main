...
const AllCallHistory = () => {
  ...
  const handleFetchCalls = async () => {
    setLoading(true);
    try {
      const { data } = await axios.get('https://www.testapi.helpubuild.in/api/v1/get-call-by-admin');
  ...
  const handleDeleteCall = async (id) => {
    setLoading(true);
    try {
      await axios.delete(`https://www.testapi.helpubuild.in/api/v1/delete-call-by-admin/${id}`);
