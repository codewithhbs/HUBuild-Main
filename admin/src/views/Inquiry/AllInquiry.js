...
const AllInquiry = () => {
    ...
    const fetchContacts = async () => {
        setLoading(true);
        try {
            const { data } = await axios.get('https://testapi.helpubuild.in/api/v1/get-contact');
    ...
    const handleUpdateStatus = async (id, status) => {
        setLoading(true);
        try {
            const res = await axios.put(`https://testapi.helpubuild.in/api/v1/update-contact/${id}`, { status });
            setContacts((prev) =>
                prev.map((contact) =>
    ...
    const handleAddNote = async (id) => {
        ...
        if (note) {
            setLoading(true);
            try {
                const res = await axios.put(`https://testapi.helpubuild.in/api/v1/add-note/${id}`, { note });
                setContacts((prev) =>
                    prev.map((contact) =>
                        contact._id === id
    ...
    const handleDeleteContact = async (id) => {
        setLoading(true);
        try {
            await axios.delete(`https://testapi.helpubuild.in/api/v1/delete-contact/${id}`);
